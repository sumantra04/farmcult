...
const CaseStudiesHeader = ({
  ...
  useGSAP(() => {
    gsap.fromTo(
      ".cs-label, .cs-main-title, .cs-main-subtitle",
      { y: 40, autoAlpha: 0 },
      {
        y: 0,
        autoAlpha: 1,
  ...
  return (
    <section ref={headerRef} className="w-full overflow-hidden">
      <div className="text-center py-[60px] md:py-[80px] px-4 md:px-[5%] max-w-[1200px] mx-auto">
        <span className="cs-label block text-[14px] md:text-[16px] font-semibold text-[#8DC83A] mb-4 uppercase">
          {tag}
        </span>
        <h1 className="cs-main-title text-[32px] md:text-[48px] leading-[42px] md:leading-[66px] font-medium text-black mb-4">
          {title}
        </h1>
        <p className="cs-main-subtitle text-[16px] md:text-[18px] text-gray-600 max-w-[700px] mx-auto">
          {subtitle}
        </p>
      </div>
    </section>
